//! X-ORDERED helpers: strip and reconstruct the OpenLDAP `X-ORDERED 'VALUES'`
//! `{n}` ordering prefix. Used by `list_model` and `value_lines` for inline
//! list editing; the former modal `OrderedEditor`/`OrderedDialog` were removed
//! in Task 9 (superseded by inline `ListValueView` editing).
